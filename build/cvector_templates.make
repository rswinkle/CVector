...
ifeq ($(config),debug)
  OBJDIR     = obj/Debug/cvector_templates
  TARGETDIR  = .
  TARGET     = $(TARGETDIR)/cvector_templates
  DEFINES   += -DUSE_TEMPLATES -DDEBUG
  INCLUDES  += 
...
ifeq ($(config),release)
  OBJDIR     = obj/Release/cvector_templates
  TARGETDIR  = .
  TARGET     = $(TARGETDIR)/cvector_templates
  DEFINES   += -DUSE_TEMPLATES -DNDEBUG
  INCLUDES  += 
  CPPFLAGS  += -MMD -MP $(DEFINES) $(INCLUDES)
...
$(TARGET): $(GCH) $(OBJECTS) $(LDDEPS) $(RESOURCES)
	@echo Linking cvector_templates
	$(SILENT) $(LINKCMD)
	$(POSTBUILDCMDS)
...
clean:
	@echo Cleaning cvector_templates
ifeq (posix,$(SHELLTYPE))
	$(SILENT) rm -f  $(TARGET)
	$(SILENT) rm -rf $(OBJDIR)
else
