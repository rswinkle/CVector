...
{
	vector_f_struct* vec;
	if (!(vec = malloc(sizeof(vector_f_struct)))) {
		assert(vec != NULL);
		return NULL;
	}
	...
	if (!(vec->a = malloc(vec->capacity * sizeof(f_struct)))) {
		assert(vec->a != NULL);
		free(vec);
		return NULL;
	}
...
{
	size_t i;
	vector_f_struct* vec1 = dest;
	vector_f_struct* vec2 = src;
	
	vec1->size = 0;
	vec1->capacity = 0;
	
	/*not much else we can do here*/
	if (!(vec1->a = malloc(vec2->capacity*sizeof(f_struct)))) {
		assert(vec1->a != NULL);
		return;
	}
...
int push_f_struct(vector_f_struct* vec, f_struct* a)
{
	...
	if (vec->capacity > vec->size) {
		...
		if (!(tmp = realloc(vec->a, sizeof(f_struct)*tmp_sz))) {
			assert(tmp != NULL);
			return 0;
		}
		vec->a = tmp;
...
int reserve_f_struct(vector_f_struct* vec, size_t size)
{
	...
	if (vec->capacity < size) {
		if (!(tmp = realloc(vec->a, sizeof(f_struct)*(size+VEC_f_struct_SZ)))) {
			assert(tmp != NULL);
			return 0;
		}
		vec->a = tmp;
...
int set_capacity_f_struct(vector_f_struct* vec, size_t size)
{
	...
	if (!(tmp = realloc(vec->a, sizeof(f_struct)*size))) {
		assert(tmp != NULL);
		return 0;
	}
	vec-> a = tmp;
