...
#define CVEC_NEW_DECLS(TYPE)                                               \
  ...
  int cvec_insert_array_##TYPE(cvector_##TYPE *vec, size_t i, TYPE *a,         \
                               size_t num);                                    \
...
#define CVEC_NEW_DEFS(TYPE, RESIZE_MACRO)                                                \
  ...
  cvector_##TYPE *cvec_##TYPE##_heap(size_t size, size_t capacity) {           \
    ...
    vec->size = size;                                                          \
    vec->capacity =                                                            \
        (capacity > vec->size || (vec->size && capacity == vec->size))         \
            ? capacity                                                         \
  ...
  void cvec_erase_##TYPE(cvector_##TYPE *vec, size_t start, size_t end) {      \
    ...
    memmove(&vec->a[start], &vec->a[end + 1],                                  \
            (vec->size - 1 - end) * sizeof(TYPE));                             \
    vec->size -= d;                                                            \
  }                                                                            \
                                                                               \
  int cvec_reserve_##TYPE(cvector_##TYPE *vec, size_t size) {                  \
    ...
    if (vec->capacity < size) {                                                \
      if (!(tmp = (TYPE *)realloc(vec->a,                                      \
                                  sizeof(TYPE) * (size + CVEC_##TYPE##_SZ)))) { \
        assert(tmp != NULL);                                                   \
        return 0;                                                              \
      }                                                                        \
      vec->a = tmp;                                                            \
...
extern size_t CVEC_##TYPE##_SZ; \
 \
int cvec_##TYPE(cvector_##TYPE* vec, size_t size, size_t capacity, void(*elem_free)(void*), void(*elem_init)(void*, void*)); \
int cvec_init_##TYPE(cvector_##TYPE* vec, TYPE* vals, size_t num, void(*elem_free)(void*), void(*elem_init)(void*, void*)); \
 \
cvector_##TYPE* cvec_##TYPE##_heap(size_t size, size_t capacity, void (*elem_free)(void*), void(*elem_init)(void*, void*)); \
cvector_##TYPE* cvec_init_##TYPE##_heap(TYPE* vals, size_t num, void (*elem_free)(void*), void(*elem_init)(void*, void*)); \
 \
void cvec_##TYPE##_copy(void* dest, void* src); \
 \
...
{ \
 ...
 \
	vec->size = size; \
	vec->capacity = (capacity > vec->size || (vec->size && capacity == vec->size)) ? capacity : vec->size + CVEC_##TYPE##_SZ; \
 \
